...
| Field | Current value |
|---|---|
| Active prototype | P0 — loop closes at SMOKE and DEVELOPMENT; P0-04 metrics **wired into the runner**; features B and C switchable and exercised at both tiers; nested donor folds and the §3.2 leakage audit implemented and tested against real fits (P0-05); P0-06's selector is implemented and `delta` calibrated (D026), PROTOCOL still v1.0.1 |
| Active task | P0-06 (selector + `delta` code and calibration DONE this session; PROTOCOL amendment to v1.1 deferred, see below) |
| Next task | **P0-06 stays IN_PROGRESS** — its row requires `delta` set *in the frozen protocol*, and the amendment that sets it is described (`PROTOCOL.md` §2, `PROGRESS.md:437`, `skeleton.py:192-197`) as a P1-01 event, but the task ledger's own dependency graph has **P1-01 depending on P0-10** (the P0 gate), not on P0-06 — a tension this entry names rather than resolves (do not "fix" it by editing either dependency; that is a call for whoever closes the P0 gate). By strict dependency order the next **P0** task is **P0-07** (workflow/CI/cache/restart, depends only on P0-05, DONE); reaching the amendment and feature A still requires P0-07 through P0-10 to close the P0 gate first, unless a future session decides P1-01 can run earlier and records that decision |
| Implementing agent/session | Claude Code session, 2026-09-19 (P0-06) |
...
| Protocol version/hash | **FROZEN — v1.0.1, unchanged this session**, `docs/planning/PROTOCOL.md`, sha256 `715639895663bc74e7b864bc1cfae60c2fcd05b013e25f23016d45e86f4f9ca5` (re-verified after P0-06's changes), recorded in `ablation_plan.yaml` (`protocol.state: frozen`). `confirmation_unlocked: false` — two constants are still null in the file (`delta` §2, precision/recall threshold §5.2). `delta`'s **value is now calibrated and recorded** (D023-D026, `delta = 0.04812`), but deliberately **not** written into PROTOCOL.md: the amendment to v1.1 batches seven items and is a P1-01 event, per explicit user direction this session |
| Baseline selector specification | **IMPLEMENTED, STILL REFUSES TO RUN.** `cnmfbench/selector.py` (`select_rank`), PROTOCOL §1/§1.4/§2 exactly — 12 tests, `python -m pytest cnmfbench -q` → 241 passed. `delta` is `None` in every config file, so `select_rank(..., delta=None)` still raises, correctly: the calibrated value (D026) is calibration evidence, not yet a config value, and using it before the v1.1 amendment would violate §2's null-until-amended intent |
...
| Last update by implementing agent | P0-06 session, 2026-09-19 |
...
| P0-06 | P0 | Preregister training-only baseline rank selection and evaluation protocol | P0-04, P0-05 | IN_PROGRESS | **Specification done at S0-03; implementation and calibration done at P0-06 (2026-09-19).** `cnmfbench/selector.py` (`select_rank`) implements PROTOCOL §1/§1.4/§2 exactly, 12 tests (`test_selector.py`). `delta` is **calibrated and recorded**: 0.04812, the maximum per-(outer fold, K) silhouette sd across 4 optimizer seeds on both `base_identifiable` and `A_weak` at DEVELOPMENT tier (D023, D026). **Remaining, and it is the only thing blocking DONE:** `delta` is still `null` in `PROTOCOL.md` itself — writing it in requires the v1.1 amendment, which PROGRESS.md:458 says batches **seven** items and which three sources describe as a P1-01 event. By explicit user direction this session, that amendment was deferred rather than rushed. **Do not mark DONE until `delta` is written into a frozen PROTOCOL.md version and `select_rank` is callable in production without raising** |
