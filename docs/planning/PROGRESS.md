...
| Implementation SHA / dirty patch hash | Git-tracked source tree unmodified at pinned SHA (`git status` clean for tracked files). `docs/` (planning bundle) is **entirely untracked** — not yet committed by any session — content hash of all `docs/*` files as of this session: `6b011f7283c9ad5040c1d5739a0a9fea113aee93afd503fd1233922f3607ec24` (`find docs -type f -exec sha256sum {} \; \| sort \| sha256sum`). No commit created this session (git commits are user-authorized only). |
