...
| Field | Current value |
|---|---|
| Active prototype | P0 — loop closes at SMOKE and DEVELOPMENT; P0-04 metrics written, next is wiring them into the runner and P0-05 |
| Active task | None |
| Next task | **P0-04 (finish)** — wire `program_recovery_cosine_v1` / `usage_error_v1` into the runner; the metrics, the corruption battery and D004's two obligations are done. Then P0-05 |
...
| P0-04 | P0 | Implement four headline metrics and corruption/invariance tests | P0-03 | IN_PROGRESS | `cnmfbench/recovery.py` + `compare.py`; 29 + 9 tests. `program_recovery_cosine_v1` (Hungarian, dummy factors, unit-tagged `Spectra` that refuses an unaligned comparison — D013) and `usage_error_v1` (reuses the alignment object, refuses one from a different fit). Full corruption battery: three invariances asserted to 1e-12, four penalties asserted. `program_precision_v1`/`program_recall_v1` and the `ambiguous` count implemented **and refusing**, thresholds null per §5.2. D004's two obligations discharged. **Remaining: wire the metrics into the runner so they write rows** |
